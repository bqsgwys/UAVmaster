<template>
  <v-container>
    ...
    ><v-footer fixed color="indigo darken-2" padless>
      <v-row justify="center" no-gutters>
        ...
        <v-btn class="my-2" color="red darken-1" rounded @click="reset()">
          <v-icon>warning</v-icon>重置所有任务(会清除所有记录！)
        </v-btn>
      </v-row>
    </v-footer>
  </v-container>
...
export default {
  ...
  async created() {
    ...
    if (this.$route.params.pass) {
      ...
      this.$router.push(`/adm/${this.groupName}`);
    }
    if (
      !this.$root.secret ||
    ...
    for (let index in this.missions) {
      this.missionList[index] = this.missions[index].description;
    }
